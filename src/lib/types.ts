...
// AI Provider Settings (legacy — kept for type compatibility)
...
// 9router-style admin router config
export interface RouterProvider {
  ...
  apiKey: string;
  enabled: boolean;
  // OAuth token fields (Google, etc.)
  refreshToken?: string;
  tokenExpiresAt?: number; // Unix ms — when access_token expires
}

export interface RouterCombo {
  id: string;
  name: string;
  /** Each entry is "providerId/model-name" e.g. "openai-prod/gpt-4o" */
  models: string[];
...
export interface RouterConfig {
  ...
  /** Direct single-model activation — "providerId/modelName". Used when no combo is needed. */
  activeModel?: string;
}
