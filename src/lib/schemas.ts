...
// Schemas for Find Jobs Flow (Simulated Job Search)
export const FindJobsInputSchema = z.object({
  keywords: z.string().min(3, "Keywords must be at least 3 characters long.").describe("Keywords for the job search, e.g., 'qualitative research assistant', 'software engineer'."),
  location: z.string().optional().describe("Optional location for the job search, e.g., 'england', 'California', 'Remote'."),
});
...
export const FindJobsOutputSchema = z.object({
  jobPostings: z.array(
    z.object({
      ...
      company: z.string().describe('A realistic but fictional company name.'),
      requirementsSummary: z.string().describe('A concise 1-3 sentence summary of the key skills and experience needed. This will be used as the "job description" for matching and tailoring.'),
      deadlineText: z.string().describe('A textual representation of the application deadline. This MUST represent a future deadline or an open status.'),
      location: z.string().optional().describe('A plausible fictional location (e.g., "Remote, USA", "London, UK"). Should align with user input if provided.'),
      jobUrl: z.string().url().optional().describe('A fictional but plausible-looking and well-formed URL for the job posting.'),
    })
  ).describe("List of plausible fictional job postings generated by the AI."),
});
